...
export default function FeesCheckoutStep({
  ...
  useEffect(() => {
    ...
    const mapped: LocalRow[] =
      feePreviewQuery.data.lineItems.map((item: any) => {
  ...
  const updateMonths = (index: number, months: number) => {
    setRows((prev) =>
      prev.map((row, i) => {
        if (i !== index) return row;

        const total = (row.unitAmount ?? 0) * months;
        return {
          ...row,
          monthsToPay: months,
          totalObligation: total,
  ...
  return (
    <Card className="space-y-6 p-6">
      <div>
        <h2 className="text-xl font-semibold">Fees & Checkout</h2>
        <p className="text-sm text-muted-foreground">
          Review fee obligations and collect initial payment.
        </p>
      </div>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>Amount Paying Now</TableHead>
          ...
          <TableBody>
            {rows.map((row, index) => (
              <TableRow key={row.feeType}>
                <TableCell>
                  <div>
                    <div className="font-medium">{row.feeType}</div>
                ...
                <TableCell>
                  {row.feeType === "TUITION" ? (
                    <Select
                      value={String(row.monthsToPay)}
                      onValueChange={(value) =>
                        updateMonths(index, Number(value))
                      }
                    >
                ...
                <TableCell>₹{row.totalObligation.toLocaleString()}</TableCell>
            ...
            <TableRow className="bg-muted/40 font-medium">
              ...
              <TableCell>₹{totals.grandTotal.toLocaleString()}</TableCell>
              <TableCell>₹{totals.payingNow.toLocaleString()}</TableCell>
              <TableCell>₹{totals.balance.toLocaleString()}</TableCell>
            </TableRow>
          </TableBody>
        </Table>
      </div>
