...
export default function Step3FeesCheckout({
  state,
  setState,
}: Step3FeesCheckoutProps) {
  ...
  const feeQ = useQuery({
    ...
    queryFn: () =>
      getFeePreview({
        ...
        academicYearId: String(state.setupData.academicYearId!),
      }),
    enabled: !!(
      state.setupData.classSectionId &&
      state.setupData.academicYearId
  ...
  // ────────────────────────────────────────────────────────────────────────────
  // Seed payments from fee preview
  // ────────────────────────────────────────────────────────────────────────────
  useEffect(() => {
    if (!feeQ.data?.lineItems) return;
 
    const rows: PaymentRow[] =
      feeQ.data.lineItems.map((item: any) => {
        ...
        return {
          ...
          calculation: isTuition
            ? ''
            : item.label ?? '',
 
          unitAmount: isTuition
            ? unitAmount
            : undefined,
 
          monthsToPay: isTuition
            ? 3
            : undefined,
 
          totalObligation: totalAmount,
 
          amountPaid: isTuition
            ? unitAmount * 3
            : totalAmount,
        };
      });
  ...
  const grandTotalObligation = payments.reduce(
    (s, p) => s + (p.totalObligation ?? 0),
    0
  );
 
  const grandTotalPaying = payments.reduce(
    (s, p) => s + (p.amountPaid ?? 0),
    0
  );
 
  const grandBalance =
    grandTotalObligation - grandTotalPaying;
 
  // Labels
  const classLabel =
    (state.setupData as any).classLevelName ??
    `Class ${state.setupData.classLevelId}`;
 
  const yearLabel =
    (state.setupData as any).academicYearName ??
    ...
    return (
      <div className="space-y-5">
        <h2 className="text-xs font-semibold tracking-widest text-gray-400 uppercase">
          Fee Preview — {classLabel} · {yearLabel}
        </h2>
 
        <div className="rounded-xl border border-gray-700 bg-gray-800/30 px-5 py-8 text-center text-gray-400">
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-gray-700 overflow-hidden">
        ...
        {payments.map((payment, idx) => {
          ...
          const badgeClass =
            feeTypeBadge[payment.feeType] ??
            feeTypeBadge.OTHER;
 
          const balance =
            (payment.totalObligation ?? 0) -
          ...
          return (
            ...
            >
              ...
              <div>
                ...
                >
                  {payment.feeType.charAt(0) +
                    payment.feeType.slice(1).toLowerCase()}
                </span>
              </div>
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        const unit =
                          ...
                          (
                            payment.amountPaid /
                            (payment.monthsToPay ?? 1)
                          );
 
                        setState((prev) => {
 
                          const updated =
                            [...prev.initialPayments] as any[];
                          ...
                          return {
                            ...prev,
                            initialPayments: updated,
                          };
                        });
                      }}
                      className="bg-gray-700 border border-gray-600 text-gray-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 appearance-none cursor-pointer"
                    >
              ...
              <div className="text-right">
                ...
                <input
                  ...
                  className="w-28 bg-gray-800 border border-gray-600 text-gray-100 rounded-lg px-3 py-1.5 text-sm text-right focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent"
                />
        ...
        <div className="grid grid-cols-[1fr_1.4fr_1fr_1fr_1fr] gap-0 px-5 py-4 border-t border-gray-600 bg-gray-800/50 items-center">
          ...
          <div className="col-span-2 text-sm font-bold text-gray-100">
            Grand total
          </div>
 
          <div className="text-sm font-bold text-gray-100 text-right">
            {fmtINR(grandTotalObligation)}
          </div>
 
          <div className="text-sm font-bold text-violet-300 text-right">
            {fmtINR(grandTotalPaying)}
          </div>
